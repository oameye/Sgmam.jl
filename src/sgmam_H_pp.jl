...
function update_p!(p, lambda, x, xdot, H_p, invH_pp)
  ...
  lambda .= sqrt.(sum(b_ .^ 2 .* invHpp_; dims=1) ./ sum(xdot .^ 2 .* invHpp_; dims=1))
  lambda[1] = 0
  lambda[end] = 0
...
function update_x!(x, λ, p′, x′′, Hx, iH, ϵ) # 63% of time
  ...
  for dof in 1:Nx
    rhs = @. (
      x[dof, idxc] +
      ϵ * (
        λ[idxc] * p′[dof, idxc] + Hx[dof, idxc] - iH[dof, idxc] * λ[idxc]^2 * x′′[dof, idxc]
      )
    )
...
function update!(x, xdot, p, pdot, lambda, H_x, H_p, invH_pp, ϵ)
  ...
  # reparametrize to arclength
  return interpolate_path!(x, alpha, iH, s) # 14% of time
end

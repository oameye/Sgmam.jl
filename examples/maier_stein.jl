...
include("systems/maier_stein.jl")
...
x_min, S_min, lambda, p, xdot = sgmam(
    sys, x_initial; iterations=1_000, ϵ=0.01, show_progress=false
)
@show S_min;
plot(x_initial[1, :], x_initial[2, :], label = "init", lw=3, c=:black)
plot!(x_min[1, :], x_min[2, :], label = "MLP", lw=3, c=:red)

@benchmark $sgmam($sys, $x_initial, iterations=100, ϵ=0.01, show_progress=false)
